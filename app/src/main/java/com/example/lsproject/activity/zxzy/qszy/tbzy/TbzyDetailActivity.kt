...
import kotlinx.android.synthetic.main.activity_tbzy_detail.*
import java.util.ArrayList


class TbzyDetailActivity : BaseActivity() {
    private var mAdapter: LessionEvaAdapter? = null
    private var title = ""

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_tbzy_detail)
        setLeftBtn(true)
        title = intent.getStringExtra("title")
        setTextTitle(title)

        setRightBtn(true,"评价", View.OnClickListener {
            var intent= Intent(this@TbzyDetailActivity, EditEvaActivity::class.java)
            intent.putExtra("title",title)
            intent.putExtra("flag","tbzy")
    ...
    private fun setData() {
        ...
        var headView = LayoutInflater.from(this).inflate(R.layout.header_tbzy_detail, null)


        pullToRefreshListView.refreshableView.addHeaderView(headView)
        pullToRefreshListView.mode = PullToRefreshBase.Mode.DISABLED
        if (mAdapter == null){
            mAdapter = LessionEvaAdapter(pxjhBeanList,this@TbzyDetailActivity,title)
